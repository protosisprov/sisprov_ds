...
//    $obj->validar_session($_SESSION);
    ...
    $titulo = "Relacion de Viviendas Censadas";
    
    $cuadro =$obj->mostrar_viviendas_censadas($_GET);              
...
//***************************************** PAGINA 1 *****************************************************************************
    ...
    $pdf->SetX($Xaux);
    $pdf->SetWidths(array(100,100,150,80,70,220));
    $pdf->Row(array('Nombre del Desarrollo','Unidad Familiar','Direccion', 'Nacionalidad', 'Cedula','Nombre'), false, 'DF');
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFillColor(255,255,255);
...
//        $c_disponible+= $c1['unidad_disponible'];
        ...
        $pdf->SetX($Xaux);
        $pdf->Row(array(utf8_decode($c1['nombre_desarrollo']),utf8_decode($c1['unidad_familiar']),utf8_decode($c1['direccion']),$c1['descripcion'],$c1['cedula'],$c1['nombre_completo']), false, 'DF');
...
// 
    ...
    $pdf->Output('Resumen de viviendas censadas.pdf', 'I'); 
?>