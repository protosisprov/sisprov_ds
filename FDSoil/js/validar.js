...
//    if ((document.getElementById(id_campo).value != '') && (document.getElementById(id_campo).value != '0')){ CAMBIADO POR MEJORAR
    ...
    document.getElementById(id_div_msj).className="alert-danger";
...
function val_checkbox(array_id_element,id_div_msj,msj){
    var retorno = false;
    for(var i in array_id_element)                        
        if(document.getElementById(array_id_element[i]).checked) 
            retorno = true;    
    document.getElementById(id_div_msj).innerHTML =(retorno == false)?msj:'';
    return retorno;
}
function valida_cantidad(a){
    var elemento = traer_element(a);
    var longitud = elemento.value.length;

    if (!IsNumeric(elemento.value)) {
        if (longitud==1) {
            elemento.value = "";
        } else {
            var valor = elemento.value.substring(0,(longitud-1));
            elemento.value = valor;
        }
    } else {
        if ((longitud==1) && (elemento.value==".")){
            elemento.value = "";
        }; 
        var puntos = 0;  
...
function traer_element(idobjeto){
...
function validatePassWord(passWord){
 ...
 var respFinal=false;
    ...
    if (passWord.length>5 && passWord.length<11)
        acepLen=true;
    if (acepNum==true && acepCha==true && acepEsp==true && acepLen==true)
        respFinal=true; 
    return respFinal;        
}
...
function FormatoTelefono(d){
    var patron_telef = new Array(4,3,2,2);
    var sep='-';
    var nums=true;
    var pat=patron_telef;
if(d.valant != d.value){
	val = d.value
	largo = val.length
	val = val.split(sep)
...
function addHeader(id, rHeader){
 $(id).load("/h"+rHeader+"-");
}
function exit(path){
    var ruta ='../../../FDSoil/modulos/admin_acceso/';
    window.location.href=ruta + path;
}

//FUNCION QUE VALIDA SOLO TEXTO EN EL CAMPO DUERANTE LA ESCRITURA
...
//FUNCIONES QUE DA FORMATO DE MILES Y DECIMALES CON . Y ,
...
function validarEmail(email, div ) {
    ...
    if ( !expr.test(email) ){
        document.getElementById(div).innerHTML="Formato del correo invalido";
        document.getElementById('validar_correo').value=false;
        document.getElementById(email).focus();
    }else{
        document.getElementById('validar_correo').value=true;
    } 
}