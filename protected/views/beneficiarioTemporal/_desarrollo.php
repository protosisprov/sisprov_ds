...
Yii::app()->clientScript->registerScript('desarollo', "
   ...
   /*  +++++++++++++++++++++++++++++++++++++++ */
    ...
    $('#BeneficiarioTemporal_unidad_habitacional_id').change(function(){
        id_unidad_habiatacional = $('#BeneficiarioTemporal_unidad_habitacional_id').val();
        $.ajax({
  ...
  /*  --------------------------------------------------------------------------- */
    ...
    $('#Tblestado_clvcodigo').change(function(){
        html = '<option value>SELECCIONE</option>';
        $('#Tblmunicipio_clvcodigo').html(html);
        $('#Tblparroquia_clvcodigo').html(html);
        $('#Desarrollo_id_desarrollo').html(html);
        $('#BeneficiarioTemporal_unidad_habitacional_id').html(html);
        $('#BeneficiarioTemporal_piso').html(html);
        $('#BeneficiarioTemporal_vivienda_nro').html(html);
        $('#BeneficiarioTemporal_tipo_vivienda').html(html);
        $('#Desarrollo_urban_barrio').val('');
        $('#Desarrollo_zona').val('');
        $('#Desarrollo_av_call_esq_carr').val('');
    });
    ...
    $('#BeneficiarioTemporal_piso').change(function(){
        html = '<option value>SELECCIONE</option>';
        $('#BeneficiarioTemporal_tipo_vivienda').html(html);
    });
...
<div class="row">       
    <div class='col-md-4' id="piso_vivienda_select" style="display: none">
        <?php
        ...
        echo $form->dropDownListGroup($model, 'vivienda_nro', array('wrapperHtmlOptions' => array('class' => 'col-sm-12 limpiar'),
            'widgetOptions' => array(
                ...
                'htmlOptions' => array('empty' => 'SELECCIONE',
                    'ajax' => array(
                        'type' => 'POST',
                        'url' => CController::createUrl('ValidacionJs/TipoBuscarTipoVivienda'),
                        'update' => '#' . CHtml::activeId($model, 'tipo_vivienda'),
                    ),
                ),
            )
                )
