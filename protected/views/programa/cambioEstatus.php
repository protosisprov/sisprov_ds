...
Yii::app()->clientScript->registerScript('anulacion', "
    $('#btn-estatus').click(function(){
        ...
        idSolicitud = '".$model."';
        if(valueAnulacion == ''){
            bootbox.alert('Por favor cambie el estatus del Programa.');
            return false;
        }
        $.ajax({
            url: '" . Yii::app()->createAbsoluteUrl('ValidacionJs/CambioEstPrograma') . "',
            async: true,
            type: 'POST',
            data: 'value='+valueAnulacion+ '&idSolicitud=' + idSolicitud,
            success: function(data){
                if(data == 1){
                    $(location).attr('href', '".$this->createUrl('/programa/create')."');
                }
            },
        });
...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">Cambio de Estatus del Programa N°<?= $model ?></h4>
        </div>
            ...
            <div class="well-large">
                <div class="col-lg-12">
                    ...
                    <select id="cambioEstatu" class="form-control" name="TestForm[dropdown]" placeholder="Select list">
                        <?php
                        $estatus = Maestro::FindMaestrosByPadreSelect(46);
